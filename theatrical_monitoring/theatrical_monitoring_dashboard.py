...
class EventLogWidget(Static):
    ...
    def log_event(self, event: TheatricalEvent):
        ...
        # Check for A2A communication patterns
        message = event.message
        is_a2a = False
        
        # Detect markdown headers and A2A patterns
        if any(pattern in message for pattern in ["##", "# ", "Task:", "Implementation:", "Summary:"]):
            is_a2a = True
            # Clean up markdown headers
            message = re.sub(r'##+\s*', '', message)
            message = message.replace('\n', ' ').strip()
            
            # Truncate long A2A messages
            if len(message) > 100:
                message = message[:97] + "..."
        
        # Format message with initials in parentheses
        if is_a2a:
            # For A2A messages, just show the agent initial with the icon
            formatted_message = f"{timestamp}({initial}) 📨 {message}"
        else:
            formatted_message = f"{timestamp}({initial}) {message}"
...
class TheatricalMonitoringApp(App):
    ...
    DataTable > .datatable--body > .datatable--row:last-child {
        background: $surface;
        text-style: bold;
    }
    ...
    .event-orchestrator { color: $primary; }
    .event-cto { color: cyan; }
    .event-backend { color: $success; }
    .event-frontend { color: magenta; }
    .event-qa { color: $warning; }
    .event-devops { color: $error; }
    .event-default { color: $text; }
    
    /* Removed colored borders - now using colored squares instead */
    """
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container():
            # PROJECT DESCRIPTION - MULTI-LINE WITH FULL DETAILS
            project_description = """🎯 Project: Real-time Chat Application with WebSocket support
💡 Features: User authentication, message history, real-time messaging
🏗️ Architecture: FastAPI backend, React frontend, PostgreSQL database
🎭 Team: Sarah (CTO), Marcus (Backend), Emily (Frontend), Alex (QA), Jordan (DevOps)"""
            self.project_widget = Static(project_description, id="project-info")
            ...
            with TabbedContent():
                with TabPane("Agents", id="agents-tab"):
                    ...
                    with Vertical():
                        ...
                        # Agent status panels with colored squares and initials
                        # System Agent first
                        self.agent_widgets["orchestrator"] = AgentStatusWidget(
                            "orchestrator", "🟦 System (SYS)"
                        )
                        self.agent_widgets["orchestrator"].classes = "agent-panel"
                        yield self.agent_widgets["orchestrator"]

                        self.agent_widgets["cto-001"] = AgentStatusWidget(
                            "cto-001", "⬜ Sarah Chen - CTO"
                        )
                        ...
                        self.agent_widgets["backend-001"] = AgentStatusWidget(
                            "backend-001", "🟩 Marcus Chen - Backend"
                        )
                        ...
                        self.agent_widgets["frontend-001"] = AgentStatusWidget(
                            "frontend-001", "🟪 Emily Rodriguez - Frontend"
                        )
                ...
                with TabPane("Performance", id="performance-tab"):
                    with Vertical():
                        ...
                        self.performance_table.add_columns("Agent", "Name", "Tasks", "Time (s)", "Cost ($)", "Tokens", "Avg/Task")
                        # Initialize with agent rows using consistent color circles - System first
                        agents = [
                            ("🟦 System/Setup", "System", "0", "0.0", "0.0000", "0", "0.0"),
                            ("", "", "", "", "", "", ""),  # Empty row for spacing
                            ("⬜ CTO", "Sarah Chen", "0", "0.0", "0.0000", "0", "0.0"),
                            ("", "", "", "", "", "", ""),  # Empty row for spacing
                            ("🟩 Backend", "Marcus Chen", "0", "0.0", "0.0000", "0", "0.0"),
                            ("", "", "", "", "", "", ""),  # Empty row for spacing
                            ("🟪 Frontend", "Emily R.", "0", "0.0", "0.0000", "0", "0.0"),
                            ("", "", "", "", "", "", ""),  # Empty row for spacing
                            ("🟧 QA", "Alex T.", "0", "0.0", "0.0000", "0", "0.0"),
                            ("", "", "", "", "", "", ""),  # Empty row for spacing
                            ("🟥 DevOps", "Jordan Kim", "0", "0.0", "0.0000", "0", "0.0"),
                            ("", "", "", "", "", "", ""),  # Empty row for spacing
                            ("────────────", "─────────", "─────", "─────────", "─────────", "──────", "─────────"),  # Separator line
                            ("📊 TOTAL", "All Agents", "0", "0.0", "0.0000", "0", "0.0"),
                        ]
    ...
    async def start_demo(self):
        ...
        # UPDATE WITH FULL DETAILED PROJECT DESCRIPTION
        try:
            project_static = self.query_one("#project-info", Static)
            detailed_description = f"""🎯 Project: {project}
💡 Features: User authentication, message history, real-time messaging
🏗️ Architecture: FastAPI backend, React frontend, PostgreSQL database
🎭 Agent Team: CTO, Backend Dev, Frontend Dev, QA Engineer, DevOps Engineer"""
            project_static.update(detailed_description)
            logger.info("DETAILED PROJECT: Updated with full description")
            # Removed notification - project details now load on startup
        except Exception as e:
            logger.error(f"Failed detailed project update: {e}")
            self.notify("❌ Project update failed!", severity="error")

        # Reset displays and start with real activities
        for widget in self.agent_widgets.values():
            # Clear previous activities and add welcome messages
            widget.clear_log()
            widget.add_activity("🎭 Theatrical Demo Starting...")
            widget.add_activity("📋 Activity log ready - scroll to see full history")
            widget.add_activity("⚡ Waiting for orchestration to begin...")
            widget.update_status("ready", 0, "Demo starting - waiting for initialization...")
    ...
    def export_conversation(self):
        ...
        filename = f"exports/aiosv3_conversation_{timestamp}.json"
    ...
    def save_log(self):
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"logs/theatrical_log_{timestamp}.txt"
    ...
    async def _run_orchestration(self):
        ...
        try:
            ...
            # Update project description with full details during orchestration
            try:
                project_static = self.query_one("#project-info", Static)
                detailed_description = f"""🎯 Project: {project}
💡 Features: User authentication, message history, real-time messaging
...
🎭 Agent Team: CTO, Backend Dev, Frontend Dev, QA Engineer, DevOps Engineer"""
                project_static.update(detailed_description)
                logger.info("Updated project with full details for orchestration")
            except Exception as e:
            ...
            try:
                ...
                if current_event_count > last_event_count:
                    ...
                    for event in new_events:
                        # Process all updates immediately in one batch
                        # Log event
                        if self.event_log:
                    ...
                    # Force UI refresh after all events processed
                    self.refresh()
                ...
                await asyncio.sleep(0.05)  # Check for updates 20 times per second for real-time feel
    ...
    def action_export_performance(self) -> None:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"exports/aiosv3_performance_{timestamp}.csv"
    ...
    def _update_performance_table(self):
        ...
        try:
            # Agent mapping for table rows (accounting for spacing rows) - System first
            agent_row_map = {
                "orchestrator": 0,     # System/Setup is first
                "cto-001": 2,         # Skip empty row
                "backend-001": 4,     # Skip empty rows
                "frontend-001": 6,    # Skip empty rows
                "qa-001": 8,          # Skip empty rows
                "devops-001": 10      # Skip empty rows
            }
            ...
            for agent_id, widget in self.agent_widgets.items():
                if agent_id in agent_row_map:
                    ...
                    self.performance_table.update_cell_at(
                        (row_index, 2), str(widget.tasks_completed)  # Tasks
                    )
                    self.performance_table.update_cell_at(
                        (row_index, 3), f"{widget.total_time:.1f}"  # Time
                    )
                    self.performance_table.update_cell_at(
                        (row_index, 4), f"{widget.total_cost:.4f}"  # Cost
                    )
                    self.performance_table.update_cell_at(
                        (row_index, 5), str(widget.total_tokens)  # Tokens
                    )
                    self.performance_table.update_cell_at(
                        (row_index, 6), f"{avg_time:.1f}s"  # Avg per task
                    )
            ...
            # System/Setup row is actually row 10 now (accounting for spacing)
            # No update needed - it's handled by the orchestrator widget
            
            # Update totalization row (row 13 - after separator)
            avg_total_time = display_time / total_tasks if total_tasks > 0 else 0.0
            
            self.performance_table.update_cell_at((13, 2), str(total_tasks))
            self.performance_table.update_cell_at((13, 3), f"{display_time:.1f}")
            self.performance_table.update_cell_at((13, 4), f"{total_cost:.4f}")
            self.performance_table.update_cell_at((13, 5), str(total_tokens))
            self.performance_table.update_cell_at((13, 6), f"{avg_total_time:.1f}s")
